...
class hikashopCartClass extends hikashopClass {
	...
	public function get($element, $default = null) {
		...
		$from_cache = isset(self::$cache['get'][$element]);
	...
	public function delete(&$elementsToDelete) {
		...
		if(!$this->app->isAdmin()) {
			...
			$query = 'SELECT cart_id FROM '.hikashop_table('cart').
				' WHERE ((user_id = ' . (int)@$this->user->user_id.') OR (session_id = ' . $this->db->Quote($jsession->getId()) . ')) AND cart_id IN (' . implode(',', $elements) . ')';
			$this->db->setQuery($query);
			if(!HIKASHOP_J25)
	...
	public function save(&$element) {
		...
		if(!$this->app->isAdmin() && !empty($element->cart_id)) {
			...
			if(!empty($old_cart->cart_user_id) && (empty($this->user) || $old_cart->cart_user_id != $this->user->user_id))
				return false;

			if(empty($old_cart->cart_user_id) && empty($this->user)) {
				$jsession = JFactory::getSession();
		...
		if(isset($element->cart_billing_address_id))
			$element->cart_billing_address_id = (int)$element->cart_billing_address_id;
		...
		if(isset($element->cart_shipping_address_ids))
			$element->cart_shipping_address_ids = (int)$element->cart_shipping_address_ids;
		...
		if(!empty($sql_data)) {
			$query = 'INSERT INTO ' . hikashop_table('cart_product') . ' ('.implode(',', array_keys($insert_data_struct)).') VALUES ('.implode('), (', $sql_data).')';
			$this->db->setQuery($query);
			$this->db->query();
		}
	...
	public function sessionToUser($cart_id, $session, $user_id, $set_current = true) {
		...
		if(!empty(self::$cache['get'][$cart_id]->user_id))
			return false;

		$query = 'UPDATE '.hikashop_table('cart').' SET user_id = ' . (int)$user_id .', session_id = '.$this->db->Quote($new_session).
				' WHERE cart_id = '.(int)$cart_id.' AND user_id = 0 AND session_id = '.$this->db->Quote($session);
		$this->db->setQuery($query);
		$ret = $this->db->query();
	...
	protected function checkQuantities(&$products, &$cart, $options = array()) {
		...
		$query = 'SELECT p.*, pp.product_published as parent_product_published, pp.product_quantity as parent_product_quantity, '.
				...
				' pp.product_access as parent_product_access '.
		...
		foreach($products as $k => &$product) {
			...
			if(!$wishlist) {
				...
				if(empty($limits['max']) && (int)$product['data']->product_quantity > 0 && (int)$product['data']->product_parent_id == 0)
					$limits['max'] = true;
			}
		}
		unset($product);
		...
		if(!empty($limits['min']) || !empty($limits['max'])) {
			...
			foreach($products as $k => &$product) {
				...
				if(!empty($product['data']->product_parent_id) && isset($counts[ (int)$product['data']->product_parent_id ])) {
	...
	protected function checkCartQuantities(&$cart, $parent_products) {
		...
		foreach($cart->products as $cart_product_id => &$product) {
			...
			if(!$wishlist && (int)$product->product_quantity != -1 && (int)$product->product_quantity < (int)$product->cart_product_quantity) {
				$notUsable[$cart_product_id] = array('id' => $cart_product_id, 'qty' => 0);
				$cart->messages[] = array('msg' => JText::sprintf('NOT_ENOUGH_STOCK_FOR_PRODUCT', $product->product_name), 'product_id' => $product->product_id, 'type' => 'notice');
				continue;
			}

			if(!$wishlist && !empty($product->product_sale_start) && (int)$product->product_sale_start > time()) {
				$notUsable[$cart_product_id] = array('id' => $cart_product_id, 'qty' => 0);
				$cart->messages[] = array('msg' => JText::sprintf('PRODUCT_NOT_YET_ON_SALE', $product->product_name), 'product_id' => $product->product_id, 'type' => 'notice');
				continue;
			}

			if(!$wishlist && !empty($product->product_sale_end) && (int)$product->product_sale_end < time()) {
				$notUsable[$cart_product_id] = array('id' => $cart_product_id, 'qty' => 0);
				$cart->messages[] = array('msg' => JText::sprintf('PRODUCT_NOT_SOLD_ANYMORE', $product->product_name), 'product_id' => $product->product_id, 'type' => 'notice');
				continue;
			}
			...
			if((int)$product->product_parent_id > 0 && $product->product_type == 'variant') {
				...
				$rem = $rem || empty($parent_product->product_published);
				$rem = $rem || ((int)$product->product_quantity == -1 && (int)$parent_product->product_quantity == 0);
				$rem = $rem || (empty($product->product_sale_start) && (int)$parent_product->product_sale_start > 0 && (int)$parent_product->product_sale_start > time());
				$rem = $rem || (empty($product->product_sale_end) && (int)$parent_product->product_sale_end > 0 && (int)$parent_product->product_sale_end < time());
		...
		if(!empty($limits['min']) || !empty($limits['max'])) {
			...
			foreach($cart->products as $cart_product_id => &$product) {
				...
				$i = (int)$product->product_id;
				$qty = $counts[ $i ];
				if(!empty($product->product_parent_id) && isset($counts[ (int)$product->product_parent_id ]))
					$qty = $counts[ (int)$product->product_parent_id ];


				if($qty < $min || ($max > $min && $qty > $max) || ($max > 0 && $min > $max)) {
					$notUsable[$cart_product_id] = array('id' => $cart_product_id, 'qty' => 0);
					$cart->messages[] = array('msg' => JText::sprintf('NOT_ENOUGH_STOCK_FOR_PRODUCT', $product->product_name), 'product_id' => $product->product_id, 'type' => 'notice');
					continue;
				}
		...
		$dispatcher->trigger('onAfterProductCheckQuantities', array(&$cart, $parent_products, &$ret) );
	...
	public function frontSaveForm() {
		...
		if(isset($formData['products'])) {
			...
			$saveCart = $this->updateProduct($cart, $updatingProducts);
		}
