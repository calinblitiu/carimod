...
class hikashopOrderClass extends hikashopClass {
	...
	public function save(&$order) {
		...
		if(!$do){
			$new_messages = $app->getMessageQueue();
			if(count($new_messages) <= count($old_messages)){
				$app->enqueueMessage('A plugin cancelled the creation/update of the order without displaying any error message.');
			}
			return false;
		}
		...
		}elseif(isset($order->order_status) && !empty($order->old) && $order_type == 'sale') {
		...
		if($file->file_free_download == 0 && $product_quantity > 1 && (substr($file->file_path, 0, 1) == '@' || substr($file->file_path, 0, 1) == '#')) {
			if(empty($file->file_pos)) {
				for($i = 1; $i <= $product_quantity; $i++) {
					$f = clone($file);
					$f->file_pos = $i;
					$id = $file->file_ordering.'_'.$file->file_id.'_'.$i;
