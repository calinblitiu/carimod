...
class plgHikashoppaymentbf_rbsbusinessgateway extends hikashopPaymentPlugin
{
	...
	function checkPaymentDisplay(&$method, &$order) {
		...
		} elseif(@$this->user->user_tester == 'Y') {
			return false;
		}
	...
	function addAddress($user, $order, $address_type, &$vars, $prefix=null) {
		...
		if(!empty($address) ) {
			...
			if(empty($this->payment_params->address_format) ) {
				$this->payment_params->address_format = '{address_lastname}, {address_firstname}, ';
			}

			$vars[$prefix.'name'] = trim(str_replace(
				array('{address_lastname}', '{address_firstname}', '{address_middle_name}'),
				array(@$order->cart->$address_type->address_lastname, @$order->cart->$address_type->address_firstname, @$order->cart->$address_type->address_middle_name),
				$this->payment_params->address_format
			));
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id) {
		...
		foreach($order->cart->products as $product) {
			...
			if (!empty($method->payment_params->show_tax_amount)) {
				$tax_cart+=($tax_item*$product->order_product_quantity);
			} else {
				$amount_item+=$tax_item;
			}
		...
		if(!empty($order->order_shipping_price) || !empty($order->cart->shipping->shipping_name) ) {
			...
			if(empty($order->order_shipping_price) ) {
				...
				$vars["C_amount_".$i] = round($order->order_shipping_price,(int)$this->currency->currency_locale['int_frac_digits']);
			}
	...
	function onPaymentNotification(&$statuses){
		...
		if (!empty($hostError)) {
			...
			$body = str_replace('<br/>', "\r\n", JText::sprintf('NOTIFICATION_REFUSED_FROM_IP', 'Worldpay Business Gateway', $ip, 'See Hostname / IPs defined in configuration')) .
				"\r\n\r\n" .
				JText::sprintf('CHECK_DOCUMENTATION', HIKASHOP_HELPURL.'payment-rbsworldpay-error#ip') .
				$order_text;
		...
		if ($vars['transStatus'] != 'Y') {
			...
			$body = str_replace('<br/>', "\r\n", JText::sprintf('PAYMENT_NOTIFICATION_STATUS', 'Worldpay Business Gateway', $vars['payment_status'] )) .
				' ' . JText::_('STATUS_NOT_CHANGED') . "\r\n\r\n" .
				JText::sprintf('CHECK_DOCUMENTATION', HIKASHOP_HELPURL.'payment-rbsworldpay-error#status') .
				$order_text;
			...
			echo '
<meta http-equiv="refresh" content="5;url='.$return_url.'" />
<style>
.pageHeading {
...
';
		...
		if($price_check != @$vars['amount'] || $this->currency->currency_code != @$vars['currency']) {
			...
			$body = str_replace('<br/>', "\r\n", JText::sprintf('AMOUNT_RECEIVED_DIFFERENT_FROM_ORDER', 'Worldpay Business Gateway', $history->amount, $price_check.$this->currency->currency_code)) .
				"\r\n\r\n" .
				JText::sprintf('CHECK_DOCUMENTATION', HIKASHOP_HELPURL.'payment-rbsworldpay-error#amount') .
				$order_text;
		...
		$body = str_replace('<br/>', "\r\n", JText::sprintf('PAYMENT_NOTIFICATION_STATUS', 'Worldpay Business Gateway', $order_status)) .
...
';
	...
	function getPaymentDefaultValues(&$element) {
		...
		$element->payment_params->redirect_button = 'style="'.
			'background: url(\'https://secure-test.wp3.rbsworldpay.com/images/rbswp/brand.gif\') top left no-repeat;' .
