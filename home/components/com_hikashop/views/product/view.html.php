...
class ProductViewProduct extends HikaShopView {
	...
	public function listing() {
		...
		if($cart_related_only) {
			...
			if(!empty($cart->products) && count($cart->products)){
				...
				if(count($mainIds)){
					$queryDiscount = 'SELECT DISCTINCT product_related_id FROM #__hikashop_product_related WHERE product_related_type=\'related\' AND product_id IN ('.implode(',',$mainIds).')';
		...
		if(!empty($rows)) {
			...
			$q = 'SELECT * FROM '.hikashop_table('file').' WHERE file_ref_id IN ('.implode(',', $ids).') AND file_type=\'product\' ORDER BY file_ref_id ASC, file_ordering ASC, file_id ASC';
			$database->setQuery($q);
			...
			$q = 'SELECT product_id FROM '.hikashop_table('product_related').' WHERE product_related_type = '.$database->quote('options').' AND product_id IN ('.implode(',', $ids).')';
			$database->setQuery($q);
			...
			$productFields = $this->fieldsClass->getFields('display:front_listing=1', $rows, 'product');
			...
			if(hikashop_level(2)) {
				...
				if(!empty($itemFields)) {
					...
					foreach($itemFields as &$itemField) {
						...
						$isListingField = $this->params->get('display_custom_item_fields', 0) && (strpos($itemField->field_display, ';front_product_listing=1;') !== false);
	...
	public function _sortCategories(&$in, &$out, $curr = null) {
		if(empty($in))
			return;
	...
	public function compare() {
		...
		foreach($elements as $k => $element) {
			...
			$f = $fieldsClass->getFields('display:compare=1',$element,'product','checkout&task=state');
